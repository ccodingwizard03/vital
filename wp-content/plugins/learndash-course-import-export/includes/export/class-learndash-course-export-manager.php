...
class LearnDash_Course_Export_Manager {
	...
	private function get_course_data( $ids ) {
		...
		$merged_data = array_merge($header_row_courses, $course_meta_keys);
		...
		foreach ($ids as $i => $course_id) {
		    ...
		    $data_rows_courses[] = array_merge($rows_courses_data, $rows_courses_general_data);
		}
	...
	private function get_lesson_data( $lesson_ids, $lesson_course_names ) {
		...
		$merged_data = array_merge( $header_row_lessons, $lesson_meta );
		...
		foreach ( $lesson_ids as $key => $lesson_id ) {
			...
			$course_title = isset( $lesson_data['sfwd-lessons_course'] ) ? get_the_title( $lesson_data['sfwd-lessons_course'] ) : '';
			...
			$rows_lessons_data = array(
				...
				'lesson_title'   => get_the_title( $lesson_post->ID ),
			...
			$data_rows_lessons[] = array_merge( $rows_lessons_data, $rows_lessons_general_data );
		}
	...
	private function get_topic_data( $topic_ids, $topic_course_names, $topic_lesson_names ) {
		...
		$merged_data = array_merge( $header_row_topics, $topic_meta );
		...
		foreach ( $topic_ids as $key => $topic_id ) {
			...
			$rows_topics_data = array(
				...
				'topic_title'       => get_the_title( $topic_post->ID ),
			...
			$data_rows_topics[] = array_merge( $rows_topics_data, $rows_topics_general_data );
		}
		...
		foreach ( $course_ids as $course_id ) {
			...
			if ( isset( $data['steps']['h']['sfwd-lessons'] ) ) {
				...
				foreach ( $data['steps']['h']['sfwd-lessons'] as $lesson_id => $lesson_data ) {
					...
					$lesson_course_names[ $lesson_id ] = get_the_title( $course_id );
					...
					if ( isset( $lesson_data['sfwd-topic'] ) ) {
						...
						foreach ( $lesson_data['sfwd-topic'] as $topic_id => $topic_data ) {
							...
							$topic_course_names[ $topic_id ] = get_the_title( $course_id );
							$topic_lesson_names[ $topic_id ] = get_the_title( $lesson_id );
						}
					}
				}
			}
	...
	private function get_shared_lesson_ids($course_ids) {
	    ...
	    foreach ($course_ids as $course_id) {
	        ...
	        $lesson_posts = learndash_course_get_lessons($course_id, array());
	        if( ! empty( $lesson_posts ) ) {
		        ...
		        foreach ($lesson_posts as $lesson_post) {
		            ...
		            $topic_posts = learndash_course_get_topics($course_id, $lesson_id, array());
    ...
    private function check_if_prerequisite( $value ) {
        ...
        if ( isset( $value ) && is_array( $value ) ) {
            foreach ( $value as $id ) {
                $titles[] = sanitize_text_field( get_the_title( absint( $id ) ) );
            }
        }
